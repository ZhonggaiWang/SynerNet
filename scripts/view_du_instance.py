...
from model.losses import get_masked_ptc_loss, get_seg_loss, ContrastLoss_prototype_instance_bg, DenseEnergyLoss, get_energy_loss,CPCLoss,get_spacial_bce_loss,ContrastLoss_moco,ContrastLoss_prototype,get_bg_fg_contrastive_clamp_loss,get_bg_fg_contrastive_loss
...
from utils.camutils import single_instance_crop_cam,single_instance_crop_abl, cam_to_roi_mask_uncertain_assign,single_instance_crop_denoise,cam_to_label, cam_to_roi_mask2, multi_scale_cam2, label_to_aff_mask, refine_cams_with_bkg_v2,cam_to_label_resized,get_per_pic_thre,multi_scale_cam2_du_heads
...
parser.add_argument("--scales", default=(0.99, 1.1), help="random rescale in training")
...
#pretrained_load——————————————————————————————————————————————————————————————————————————————————————————————————
    ...
    trained_state_dict = torch.load('/home/zhonggai/python-work-space/ToCo-B_WSSS_work/DEFormer/DEFormer/scripts/work_dir_voc_wseg/75.8/checkpoints/default_model_iter_8000.pth', map_location="cpu")
...
#add param ——————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
    ...
    b1_contrast_prototype = ContrastLoss_prototype_instance_bg()
    b2_contrast_prototype = ContrastLoss_prototype_instance_bg()
    ...
    for n_iter in range(args.max_iters):
        ...
        try:
            img_name, inputs, cls_label, img_box, crops,image_origin,gt_mask_label = next(train_loader_iter)
...
#get pseudo label --------------------------------------------------------------------------------------------------------------------------------
        ...
        # _28x_cam =0.5 * b1_28x_cam + 0.5 * b2_28x_cam
...
#get_local_pic_to_encode------------------------------------------------------------------------------
        ...
        # input_image.save('input_image_norm.png')
        ...
        # show_mask_cam(_roi_mix_cam,cls_label,args.low_thre,args.high_thre, 'roi_mix_cam')
        
        



        # b1_local_crops, b1_flags, b1_box= single_instance_crop_denoise(images=inputs, cls_label = cls_label,roi_mask_uncertain_assign=b1_roi_mask_uncertain_assign, crop_num=n_crops, crop_size=args.local_crop_size, refine_pesudo_label= refined_roi_mix_pesudo_label, boundary_box=img_box)
        # b2_local_crops, b2_flags, b2_box= single_instance_crop_denoise(images=inputs, cls_label = cls_label,roi_mask_uncertain_assign=b2_roi_mask_uncertain_assign, crop_num=n_crops, crop_size=args.local_crop_size, refine_pesudo_label= refined_roi_mix_pesudo_label, boundary_box=img_box)
        #pseudo label
        b1_local_crops, b1_flags, b1_box= single_instance_crop_abl(images=inputs_denorm, cls_label = cls_label,roi_mask_28x=b1_roi_mask, crop_num=n_crops, crop_size=args.local_crop_size, refine_pesudo_label= gt_mask_label, boundary_box=img_box)

        #cam uncertain
        b1_roi_mask[b1_roi_mask>=0] = b1_roi_mask[b1_roi_mask>=0] + 1
        b1_local_crops, b1_flags, b1_box= single_instance_crop_cam(images=inputs_denorm, cls_label = cls_label,roi_mask_28x=b1_roi_mask, crop_num=n_crops, crop_size=args.local_crop_size, refine_pesudo_label= b1_roi_mask, boundary_box=img_box,label = b1_flags[0])
        ...
        b2_local_crops, b2_flags, b2_box= single_instance_crop_cam(images=inputs_denorm, cls_label = cls_label,roi_mask_28x=b2_roi_mask, crop_num=n_crops, crop_size=args.local_crop_size, refine_pesudo_label= refined_roi_mix_pesudo_label, boundary_box=img_box)
...
# PCA Analyse
        ...
        import matplotlib.pyplot as plt
        # 将fmap转换为numpy数组
        fmap_np = b1_fmap.squeeze().cpu().detach().numpy()  # 将张量转换为numpy数组

        # 将特征张量重新整形为[768, h*w]
        fmap_reshaped = np.reshape(fmap_np, (768, -1)).T

        # 使用PCA将特征降至3维
        pca = PCA(n_components=3)
        fmap_3d = pca.fit_transform(fmap_reshaped)


        # 可视化展示
        fig = plt.figure()
        ax = fig.add_subplot(111, projection='3d')
        ax.scatter(fmap_3d[:, 0], fmap_3d[:, 1], fmap_3d[:, 2])
        plt.savefig('Feature PCA 3D')
        plt.close()
        # 将数据重新整形为(h, w)的形状
        fmap_3d = F.sigmoid(torch.tensor(fmap_3d))
        fmap_reconstructed = np.reshape(fmap_3d, (28, 28, 3))
        plt.imshow(fmap_reconstructed)
        plt.savefig('Feature PCA')
...
# branch2 : adaptive-label-bce + cls loss part-------------------------------------------------------------------------------------------------------------------
        ...
        if n_iter <= 4000:
            ...
            # import math
            # shedule = math.cos((n_iter-4000)/4000) * 0.1
...
#show mask --------------------------------------------------------------------------------------------------------------
        ...
        # show_mask_cam(b1_cams_aux,cls_label,args.low_thre,args.high_thre, 'b1_aux_cam')
        # show_mask_cam(b2_cams_aux,cls_label,args.low_thre,args.high_thre, 'b2_aux_cam')
        ...
        # for i in range(len(b2_box)):
        #     new_box = []
        #     for item in b2_box[i]:
        #         item = max(item - 48,0)
        #         item = int(item)
        #         new_box.append(item)
        ...
        # import matplotlib.pyplot as plt
        # b1_refined_pseudo_label[b1_refined_pseudo_label == 0] = -2
        # plt.imshow(b1_refined_pseudo_label.squeeze(0).cpu(), cmap='jet', vmin=-2, vmax=20)
        # plt.colorbar()
        # plt.title("seg" + "refine")
        
        # plt.savefig('output-image'+ "/" + f'seg' + 'refine' + '.png')
        # plt.close()
...
#train------------------------------------------------------------------------------------------------------------------------
        if n_iter <= 2000:
            ...
            loss = 1.0 *  (b1_cls_loss + b2_cls_loss) + 1.0 * (b1_cls_loss_aux + b2_cls_loss_aux) + args.w_ptc * ptc_loss + args.w_seg * seg_loss + 0.1 * network_sim_loss + 0.2 * contrast_loss
        else:
            loss = (0.5 * b2_spacial_bce_loss + 0.5 * b2_cls_loss) + 1.0 * b1_cls_loss+ 1.0 * (b1_cls_loss_aux + b2_cls_loss_aux) + args.w_ptc * ptc_loss + args.w_seg * seg_loss + 0.1 * network_sim_loss + 0.2 * contrast_loss
