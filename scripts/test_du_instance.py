...
from utils.camutils import single_instance_crop_v2,single_bg_fg_crop_denoise_v3,cam_to_label, cam_to_roi_mask2, multi_scale_cam2, label_to_aff_mask, refine_cams_with_bkg_v2,cam_to_label_resized,get_per_pic_thre,multi_scale_cam2_du_heads
...
# branch2 : adaptive-label-bce + cls loss part-------------------------------------------------------------------------------------------------------------------
        ...
        b2_PaBCEloss = get_spacial_bce_loss(b2_cam_12th, cls_label, per_pic_thre)
...
#train------------------------------------------------------------------------------------------------------------------------
        if n_iter <= 2000:
            ...
            loss = 1.0 *  (b1_cls_loss + b2_cls_loss) + 1.0 * (b1_cls_loss_aux + b2_cls_loss_aux) + args.w_ptc * ptc_loss + args.w_seg * seg_loss + 0.05 * contrast_loss
        else:
            loss = (0.4 * b2_PaBCEloss + 0.6 * b2_cls_loss) + 1.0 * b1_cls_loss+ 1.0 * (b1_cls_loss_aux + b2_cls_loss_aux) + args.w_ptc * ptc_loss + args.w_seg * seg_loss + 0.05 * contrast_loss
